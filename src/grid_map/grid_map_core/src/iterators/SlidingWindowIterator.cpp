...
void SlidingWindowIterator::setup(const GridMap& gridMap)
{
  if (!gridMap.isDefaultStartIndex()) throw std::runtime_error(
      "SlidingWindowIterator cannot be used with grid maps that don't have a default buffer start index. MY EXTRA OUTPUT: This error should be fixed by adding convertToDefaultStartIndex() at line 83 in SlidingWindowMathExpressionFilter.cpp");
