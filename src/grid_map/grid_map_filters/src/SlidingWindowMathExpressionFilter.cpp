...
bool SlidingWindowMathExpressionFilter<T>::update(const T& mapIn, T& mapOut) // tried: (T& mapIn, T& mapOut)
{
  ...
  grid_map::SlidingWindowIterator iterator(mapInCopy, inputLayer_, edgeHandling_, windowSize_); // old: iterator(mapIn, inputLayer_, edgeHandling_, windowSize_);
  if (useWindowLength_) iterator.setWindowLength(mapInCopy, windowLength_); // old: iterator.setWindowLength(mapIn, windowLength_);
